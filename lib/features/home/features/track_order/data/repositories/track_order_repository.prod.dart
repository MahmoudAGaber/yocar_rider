...
class TrackOrderRepositoryImpl implements TrackOrderRepository {
  ...
  Stream<(OrderEntity, DriverLocation?)> listenToOrderUpdates() {
    ...
    return Rx.combineLatest3(
      ...
      (
        ...
        return (
          orderData.copyWith.call(chatMessages: [...orderData.chatMessages, ...messageData]),
          driverLocationData,
        );
      },
    );
  ...
  Future<Either<Failure, ChatMessageEntity>> sendMessage({
    ...
    final messageEntity = await graphQLDatasource.mutate(Options$Mutation$SendMessage(
      variables: Variables$Mutation$SendMessage(
        orderId: orderId,
        message: message,
      ),
    ));
    return messageEntity.map((r) => r.createOneOrderMessage.toEntity);
  }
