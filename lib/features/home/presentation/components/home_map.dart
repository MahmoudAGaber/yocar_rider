      ...
      listener: (context, state) {
        state.mapOrNull(
          ...
          confirmLocation: (value) {
            mapViewController?.moveCamera(value.selectedLocation.latLng2, 16);
          },
          rateDriver: (value) {
            if (value.markers.length < 2) return;
            mapViewController?.fitBounds(
      ...
      builder: (context, state) {
        return BlocBuilder<SettingsCubit, SettingsState>(
          ...
          builder: (context, settingsState) {
            print("MapMap${settingsState.mapProvider}");
            return GenericMap(
              ...
              addressResolver: state.mapViewMode == MapViewMode.static
                  ...
                  : (provider, location) async {
                      final result = await locator<GeoDatasource>().getAddressForLocation(latLng: location);
                      return result.fold((l) => Place(location, ""), (r) => Place(location, r.address));
                    },
              onMapMoved: (place) {
                state.mapOrNull(confirmLocation: (confirmLocation) {
                  if (place == null) {
                    locator<PlaceConfirmCubit>().onLoading();
                  } else {
                    locator<PlaceConfirmCubit>().onLoaded(place: place.toPlaceEntity);
                  }
                });
                if (place != null && state.mapViewMode == MapViewMode.picker) {
                  locator<HomeCubit>().onMapMoved(selectedLocation: place.toPlaceEntity);
                }
              },
